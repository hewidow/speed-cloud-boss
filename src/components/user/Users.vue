...
<!--            </el-tooltip>-->
            ...
            <el-tooltip :enterable="false" class="item" content="分配角色" effect="dark" placement="top">
              <el-button icon="el-icon-setting" size="small" type="warning" @click="setRole(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
      ...
      <!--分页区域-->
      <el-pagination
        :current-page="this.queryInfo.pageNumber"
    ...
    <el-dialog
      ...
      <el-form ref="addFormRef"
               ...
               :rules="addFormRules"
               label-width="70px">
        <!-- prop=username 对应了 addFormRules中的username校验规则,v-model对应的是下面的addForm对象-->
...
export default {
  ...
  created () {
    // this.queryInfo.pagenum = window.sessionStorage.getItem('sessionpage')
    // this.queryInfo.pagesize = window.sessionStorage.getItem('sessionpagesize')
    this.getUserList()
  },
  methods: {
    async getUserList () {
      const { data: res } = await this.$axios.post('users', this.queryInfo)
      if (res.code !== 200) {
    ...
    handleSizeChange (newSize) {
      // console.log(newSize)
      // window.sessionStorage.setItem('sessionpagesize', newSize)
    ...
    handleCurrentChange (newPage) {
      // console.log(newPage)
      // window.sessionStorage.setItem('sessionpage', newPage)
    ...
    async userStateChanged (userInfo) {
      ...
      const { data: res } = await this.$axios.post('updateUser', {
